...
exports.DEFAULT_MAX_LISTENER = 20;
...
exports.createClient = function(port, host, user, pass) {
  ...
  httpClient.setMaxListeners(exports.DEFAULT_MAX_LISTENER);
  ...
  couchClient._queueRequest = function(options, cb) {
    ...
    var
      ...
      onError = function(exception) {
        if (!cbFired) cb && cb(exception);
        cbFired = true;
        // Cannot httpClient.removeListener('error', onError) because, at least for ECONNREFUSED,
        // two events will fire, the first with an exception, and the second with the boolean true value.
      };

    httpClient.addListener('close', onClose);
    if(options.catch_error) {
      httpClient.addListener('error', onError);
    }
...
Db.prototype.getDesign = function(id, cb) {
  return this.getDoc(ensureDesignId(id), cb);
};
...
Db.prototype.getDocsByKey = function(keys, cb) {
    return this.allDocs({keys: keys}, {include_docs:true}, cb);
};
...
Db.prototype.paginate = (function() {
  var next_key = null,
    current_key = null,
    page = 1,
    per_page = 30,
  ...
  return function(design, view, query, cb) {
    if (typeof(query) === 'function' && !cb) {
      cb = query;
      query = {};
    }

    if(!query.page) { query.page = page; } else { query.page = parseInt(query.page, 10); }
    if(!query.per_page) { query.per_page = per_page; } else { query.per_page = parseInt(query.per_page, 10); }

    switch(query.page) {
      case (parseInt(page, 10) + 1):
        descending = false;
        query = mixin({
    ...
    return this.request({
      ...
      if(!err) {
        ...
        response.per_page = per_page = query.per_page;
        response.total_pages = Math.ceil(response.total_rows / response.per_page);
        current_key = response.rows[0].key;
        next_key = response.rows[response.rows.length - 1].key;
        if(descending) { var tmp = current_key; current_key = next_key; next_key = tmp; }
      }
      cb(err, response);
    });
  };
...
Db.prototype.spatial = function(design, spatial, query, cb) {
    if (typeof(query) === 'function' && !cb) {
        cb = query;
        query = undefined;
    }
    return this.request({
        path: ['', ensureDesignId(design), '_spatial', spatial].join('/'),
        query: query
    }, cb);
};
...
Db.prototype.spatiallist = function(design, list, view, query, cb) {
    if (typeof(query) === 'function' && !cb) {
        cb = query;
        query = undefined;
    }
    return this.request({
        path: ['', ensureDesignId(design), '_spatiallist', list, view].join('/'),
        query: query
    }, cb);
};
